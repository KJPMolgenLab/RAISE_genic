...
## dont get it why it does not work do manually 

testdrug = function (drug, target, MEs){  
  res=data.frame()
  for (mod in MEs){
...
drug = unique(SampleInfo$Drug)[4]

for (d in unique(SampleInfo$Drug)){
  assign(paste0("WGCNA_lmer_", d), testdrug(drug = d, target="Conz", MEs = MEnames))
}

boxplot(SampleInfo[SampleInfo$Drug=="Valporate", "MEdarkturquoise"] ~ as.factor(SampleInfo[SampleInfo$Drug=="Valporate", "Conz"]), 
